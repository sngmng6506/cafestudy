...
- **의존성 잠금 파일이 현재 없음**: Ternlight 추가 과정에서 오래된 `package-lock.json`을
  제거했고 Docker는 임시로 `npm install`을 사용한다. 배포는 가능하지만 설치 시점에 따라
  하위 의존성 버전이 달라질 수 있다. 로컬에서 `npm install`로 새 lockfile을 생성·커밋한 뒤
  Dockerfile을 다시 `npm ci`로 되돌리는 것이 남은 정리 작업이다.
