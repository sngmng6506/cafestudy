...
- **Members** — 소모임 멤버 목록, 검색, 아바타, 멤버 프로필 카드
  (포인트·활동 통계·뱃지 컬렉션)
- **Badges** — Hugging Face 기반 AI 뱃지 생성, 미리보기, 내 뱃지 적용·대표 뱃지 변경·삭제
...
- **주사위** — 굴려서 포인트 획득
- **자연어 기능 검색** — 헤더 검색 버튼에서 하고 싶은 일을 한국어 문장으로 입력하면 관련 기능을 추천.
  한국어 키워드 점수 70%와 브라우저에서 실행되는 `@ternlight/mini` 의미 점수 30%를 결합하며,
  모델이 실패하면 키워드 검색만 사용
- **검색 안내** — 더보기 휠에서 Ternlight, 검색 파이프라인, 영어 중심 모델의 한계와 잠재 이슈 설명
...
자연어 검색은 별도 서버 모델 없이 Vue 클라이언트에서 동작합니다.

```text
한국어 질의
→ 메뉴 설명·키워드·예시 문장과 한국어 키워드 매칭
→ @ternlight/mini lazy loading 및 문장 임베딩
→ 메뉴별 최고 코사인 유사도 계산
→ 키워드 70% + 의미 점수 30%
→ 상위 3개 기능 추천
→ 결과 선택 시 기존 기능 탭으로 이동
```

Ternlight는 문장을 384차원 벡터로 바꾸는 초경량 임베딩 모델이며, 답변을 생성하는 챗봇은 아닙니다.
WASM 기반으로 브라우저 CPU에서 실행하고 첫 로딩 이후 메모리에 메뉴 임베딩을 캐시합니다.
다만 영어 중심 교사 모델을 증류해 만든 모델이라 한국어 의미 검색 품질에는 편차가 있을 수 있습니다.
그래서 한국어 키워드 검색을 주 점수로 사용하고, Ternlight는 보조 순위 조정에만 사용합니다.

현재 알려진 잠재 이슈:

- 첫 의미 검색에서 모델 다운로드·초기화 지연이 생길 수 있음
- 구어체나 키워드가 거의 없는 한국어 질의는 의미 순위가 흔들릴 수 있음
- 메뉴 설명과 예시 문장이 부족하면 검색 품질이 떨어짐
- WASM이 지원되지 않거나 모델 로딩이 실패하면 의미 검색 없이 키워드 검색만 동작
...
| 문서 | 무엇을 담는가 | 언제 보는가 |
|------|--------------|-------------|
| [README.md](./README.md) | 시작하기, 기능, 스크립트, 자연어 검색 파이프라인 | 프로젝트를 처음 볼 때 |
