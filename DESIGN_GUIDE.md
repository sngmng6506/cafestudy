...
### 버튼
- **Primary**: `#03C75A` 배경, 흰 텍스트, `rounded-[10px]`, 500. hover `#02b350`.
  비활성 `disabled:opacity-50`
- **Secondary/Outline**: 흰 배경, `#333333` 텍스트, `1px solid #dadce0`, `rounded-[10px]`, hover `#f5f6f7`
- **Danger**: `#e74c3c` 테두리·텍스트 (모임 취소 등)
- **소모임 CTA**: `#7C3AED` 배경, 흰 텍스트 ("소모임에서 신청" 등 외부 이동)
- 작은 상태 배지·칩은 예외적으로 4px(`rounded`) 유지 — 버튼만 10px
...
### 카드
- 표준 카드는 `.surface-card` 유틸(`styles.css`) — 흰 배경, radius 16px, 패딩 20px,
  **테두리·그림자 없음**. 연한 페이지 배경(`#f7f8f9`) 위에 여백으로 구획한다.
  (예전의 `border + shadow-sm + p-6` 무거운 패턴을 대체 — 답답함 제거)
- 내부에서 자체 패딩을 갖는 리스트/아코디언은 `.surface-card--flush`(패딩 0)
...
- 상태 배지(모집중/마감): 모집중 = `#e9f8ef` 배경 `#03883f` 텍스트(연한 초록 pill),
  마감 = `#f5f6f7` 배경 `#5f6368` 텍스트
