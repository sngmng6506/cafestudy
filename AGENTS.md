...
- [DESIGN_GUIDE.md](./DESIGN_GUIDE.md) — 색상/타이포/컴포넌트 디자인 토큰.
...
🤖 Generated with <도구 이름> (예: Claude Code, Codex CLI)
```
...
- 마지막 줄 `🤖 Generated with <도구 이름>`은 **AI 커밋 마커**. 실제 사용한 도구
  이름을 그대로 쓴다(Claude Code / Codex CLI 등). 유지한다.
