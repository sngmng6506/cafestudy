...
onMounted(async () => {
  await Promise.all([loadNotices(), loadUsers()]);
});
...
async function resetPassword(user) {
  ...
  try {
    ...
    toast.success(`${user.name} 님의 비밀번호를 초기화했어요.`);
  } catch (error) {
    toast.error(error.message);
  } finally {
    resettingId.value = '';
  ...
  <section class="grid gap-5">
    ...
    <template v-else-if="activeSection === 'accounts'">
      ...
      <section class="surface-card">
        ...
        <p class="ui-text-muted mt-1 text-[13px]">관리자는 멤버만, 최고 관리자는 관리자와 멤버의 비밀번호를 초기화할 수 있어요.</p>
          ...
          <li v-for="user in users" :key="user.id" class="flex items-center justify-between gap-3 py-3">
            <div class="min-w-0">
              ...
              <p class="ui-text-caption mt-0.5">
                ...
                {{ user.hasPassword ? '비밀번호 사용 중' : user.requiresSetupToken ? '설정 코드 필요' : '최초 설정 전' }}
              </p>
            </div>
            <button
            ...
            <span v-else class="ui-text-caption shrink-0 text-[12px]">초기화 불가</span>
          </li>
        </ul>
      </section>
    </template>
