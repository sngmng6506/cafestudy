...
- 무인 재연결 — 태블릿이 재부팅돼 adb 포트가 바뀌어도 worker가 기기 IP를 훑어
  다시 붙는다(설정 주소 → mDNS → 포트 스캔). 무선 주소는 포트가 아니라 IP로
  기기를 식별한다.
...
- 태블릿 재부팅 뒤 무선 디버깅 자체가 켜지지 않으면 열린 adb 포트가 없어 포트
  스캔도 소용없다. `persist.adb.tcp.port`를 막는 롬에서는 사람이 태블릿 화면에서
  다시 켜야 한다.
