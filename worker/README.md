...
`내모임` 경로는 실기기로 end-to-end 검증했다. worker의 `ALLOW_SOMOIM_SUBMIT`과
서버의 `SOMOIM_AUTOMATION_ALLOW_SUBMIT`이 둘 다 켜져야 제출이 실행된다.
...
홈 화면에서는 정모 이름이다(추천·주변 모임 카드는 `groupname_text`). 그래서 세기
전에 `가입한 모임` 헤더로 화면을 먼저 확인해야 한다. 확인하지 않으면 아직 홈에
있을 때 남의 정모를 가입 모임으로 착각한다.
...
덤프에 하단 탭 바가 하나도 없어서 job이 실패한 적이 있다. 화면을 한 번만 읽고
판단하지 말고 폴링한다.
