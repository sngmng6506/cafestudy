...
- **Meetups** — 예정 모임 목록, 모임 만들기, 장소 검색, 참여/취소.
  자동화 스위치(`SOMOIM_AUTOMATION_ALLOW_SUBMIT`)가 켜져 있으면 모임을 만들 때 Bot이
  소모임 앱에도 정모를 자동 등록한다(등록 중에는 참여 불가). 스위치가 꺼진 지금은
  적용되지 않고 기존과 동일하게 동작한다
...
- **더보기 메뉴** — 탭바에 없는 기능들을 목록으로 펼쳐 보여줌
