...
// 차이나면 "외 N명"으로 보정 — 매핑 안 된 참석자 수를 감추지 않기 위함.
export function somoimAttendees(event) {
  const participantCount = Number(event.joinedCount ?? 0);
  const named = (event.attendees ?? []).map((a) => a.name).filter(Boolean);
  const unmapped = participantCount - named.length;
  return unmapped > 0 ? [...named, `외 ${unmapped}명`] : named;
}
...
// 참석자 이름 배열 → 아바타 스택 데이터.
// "외 N명" 문자열은 집계용이므로 스택에서 제외하고 overflow 카운트로 환산.
