...
제출한 뒤에는 버튼을 눌렀다는 사실을 성공으로 삼지 않는다. **소모임 앱 화면으로
돌아왔고 정모 개설 폼이 사라졌을 때만** 제출된 것으로 본다. 폼이 안 보인다고
제출된 것이 아니다 — 다른 앱 창이 폼을 덮으면 폼 노드가 덤프에서 사라지기 때문이다.

현재 앱은 정모 사진 없이 제출을 받지 않는다(실기기 확인). handler가 사진을 넣지
못하므로 submit은 여기서 `needsManualReview: true`로 멈춘다.
