...
3. 미니PC worker가 job을 가져가 소모임 앱에서 정모를 개설한다. 정원 1로 만들어
   소모임에서는 마감 상태가 되고, 아무도 그쪽에서 신청할 수 없다.
...
| `scheduledAt` | 웹 모임 일시 |
| `location` | 웹 모임 장소 |
| `capacity` | **1 고정** — 소모임에서 마감 상태로 만들어 그쪽 신청을 막는다 |
| `description` | 웹 모임 설명 + "참가 신청은 CafeStudy에서 해주세요" |
