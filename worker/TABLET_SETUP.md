...
worker는 기기를 찾지 못하면 세 단계를 차례로 시도한다.
...
3. 기기 IP의 열린 포트 전수 스캔 — 위 둘이 모두 실패했을 때의 마지막 수단이다.
...
포트가 바뀌어 붙어도 worker는 같은 IP면 같은 태블릿으로 본다. 즉 포트가 달라졌다는
이유만으로는 `ADB_SERIAL`을 고칠 필요가 없다 — 다만 다음 재연결이 한 번에 끝나도록
새 주소로 갱신해 두면 스캔 4초를 아낀다.
