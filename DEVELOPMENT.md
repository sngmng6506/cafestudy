...
- worker는 claim 직후 preflight와 최종 제출 표시에서 원본 모임의 취소 상태를 다시
  확인한다. 그 사이 경합으로 취소 뒤 생성까지 끝나더라도 완료 훅이 `delete_meetup`
  job을 만들어 양쪽 상태를 자동으로 맞춘다.
- 생성 preflight는 크롤링 데이터에 같은 제목·분 단위 일시의 정모가 있으면 새로
  만들지 않고 기존 정모를 등록 결과로 인정한다.
...
- 다른 검색 제공자로 넘어가는 폴백은 없다. 카카오가 못 찾으면 못 찾은 채로 둔다 —
  네이버는 같은 입력에 자신 있게 엉뚱한 곳을 돌려줬고, 그걸 믿으면 남의 카페에
  이력이 섞인다.
...
- 카카오가 찾지 못한 장소는 합쳐지지 않는다. 카페 이력은 `cafe_places.kakao_place_id`가
  같은 항목만 하나로 본다. 사람이 소모임 앱에 자유롭게 적은 문자열("정기모임장소 근처",
  "아비아채 지하1층")은 검색 결과가 비어 각자 남는다 — 네이버는 같은 입력에 자신 있게
  엉뚱한 곳(고양시 스터디룸)을 돌려줬고, 그걸 믿고 합치면 남의 카페에 이력이 섞인다.
- `cafe_comments.cafe_location`은 여전히 원본 문자열 키다. 장소 ID로 합쳐진 카페에서는
  한 사람이 코멘트를 두 개 가질 수 있다(`(cafe_location, user_id)` 유니크라 문자열마다
  하나씩). 코멘트를 장소 ID로 옮기려면 ID가 없는 장소의 처리를 먼저 정해야 한다.
